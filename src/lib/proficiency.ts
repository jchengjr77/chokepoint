...
/**
 * Maps a raw session count to a visual tier: 0 sessions is untrained
 * (baseline styling), then three increasingly-trained tiers driving
 * thicker node borders and edge strokes.
 */
 ...
 * logged between a pair of nodes (not by session count) — a pair with many
